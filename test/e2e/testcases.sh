...
function oci_local {
    workflow_oci_copy imageset-config-oci-copy.yaml "test-catalog-latest" "oci://${MIRROR_OCI_DIR}" -c="--use-oci-feature --oci-feature-action=copy --source-skip-tls --source-use-http --oci-insecure-signature-policy"
    workflow_oci_mirror imageset-config-oci-mirror.yaml "docker://localhost.localdomain:${REGISTRY_DISCONN_PORT}/test" -c="--use-oci-feature --oci-feature-action=mirror --dest-skip-tls --dest-use-http --oci-insecure-signature-policy"
