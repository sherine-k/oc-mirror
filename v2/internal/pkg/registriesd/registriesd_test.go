...
func TestPrepareRegistrydCustomDir(t *testing.T) {
	...
	expectedCacheRegistryFile := filepath.Join(GetCustomRegistrydConfigPath(testFolder), "localhost:55000.yaml")
	expectedDestRegistryFile := filepath.Join(GetCustomRegistrydConfigPath(testFolder), "mymirror.com.yaml")
...
func TestGetCustomRegistrydConfigPath(t *testing.T) {
	...
	registriesDirPath := GetCustomRegistrydConfigPath(testFolder)
