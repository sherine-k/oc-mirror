...
func (o *Mirror) copy(ctx context.Context, src, dest string, opts *CopyOptions) (retErr error) {
	...
	if !opts.RemoveSignatures {
		sourceCtx.RegistriesDirPath = registriesd.GetCustomRegistrydConfigPath(opts.SrcImage.global.WorkingDir)
	}
	...
	if !opts.RemoveSignatures {
		destinationCtx.RegistriesDirPath = registriesd.GetCustomRegistrydConfigPath(opts.DestImage.global.WorkingDir)
	}
